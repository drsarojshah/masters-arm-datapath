...
// Use this prototype... while using it with HLS
...
int main() {    
    ...
    /*  for loop for call function and print its result values...

    for (int pc_in ; pc_in < 8; pc_in++) {
        processor(pc_in, &main_memory_out, &opcode_out, &op_value_1_out, &op_value_2_out, &result_out);

        printf("PC = %d; Main Memory = 0x%08X; Result = %d\n\n",
               pc_in, main_memory_out, result_out);
    } 
    */
